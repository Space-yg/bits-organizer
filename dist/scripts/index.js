...
const bytesDataTextArea = document.getElementById("bytes-data");
/** Convert button */
 ...
 */
function removeHighlightingInBitsTable() {
 ...
 */
function highlightInBitsTable(startBit, length, color) {
    // Add highlighting
...
highlightAllButton.addEventListener("click", () => {
    ...
    for (let i = 0; i < organizationTable.tBodies[0].rows.length; i++) {
        ...
        const bitsInput = tr.cells[3].firstElementChild;
 ...
 */
function indexOfTableRowInTableSection(tableSection, tableRow) {
    for (let i = 0; i < tableSection.rows.length; i++) {
        if (tableSection.rows[i] == tableRow)
            return i;
    }
    return -1;
}
 ...
 */
function getBitsTds(startBit, length) {
    const bits = [];
...
function showInfo(startBit, length) {
    // Reset info
    infoDiv.innerHTML = "";
    // Get bits
    const bits = getBitsTds(startBit, length).map(td => +td.innerText);
    const decimal = parseInt(bits.map(bit => bit + "").reduce((pre, cur) => pre + cur, ""), 2);
    // Create info
    const infos = [];
    if (bits.length > 0) {
 ...
 */
function createOrganizationTableRow() {
    // <tr>
...
const addOrganizationTableDataButton = document.getElementById("add-organization-table-data");
...
function bytesToBits(bytes) {
    ...
    for (const byte of bytes)
        bits.push(...byte.toString(2).padStart(8).split("").map(bit => +bit));
    return bits;
}
/**
 ...
 */
function addBitsToBitsTable(bits) {
    let tr, td;
